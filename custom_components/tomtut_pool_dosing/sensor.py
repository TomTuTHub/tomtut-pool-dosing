...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities):
    ...
    entities: list[BinarySensorEntity] = [
        PoolFlowBinary(coordinator, entry),
...
class PoolFlowBinary(_BaseBinary):
    ...
    def __init__(self, coordinator, entry: ConfigEntry):
        ...
        self._attr_name = "TomTuT Pool Dosieranlage Flow"
        self._attr_unique_id = f"{entry.entry_id}_flowswitch"

        # ✅ exakt deine gewünschte Entity-ID
        self.entity_id = "binary_sensor.tomtut_pool_dosieranlage_flow"

    @property
    def is_on(self):
        data = (self.coordinator.data or {}).get("measurements", {})
        value = (data.get("flowswitch", {}) or {}).get("value")
        if value is None:
            return None
        return value == 1
